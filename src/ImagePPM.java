...
public class ImagePPM {
    ...
    public static ImagePPM PGMToPPM(Image pgmImage) {
        ImagePPM outputPGM = new ImagePPM(3, pgmImage.width, pgmImage.height);

        for (int y = 0; y < pgmImage.height; y++) {
            for (int x = 0; x < pgmImage.width; x++) {
                outputPGM.pixels[0][x][y] = pgmImage.pixels[x][y];
                outputPGM.pixels[1][x][y] = pgmImage.pixels[x][y];
                outputPGM.pixels[2][x][y] = pgmImage.pixels[x][y];
            }
        }
    ...
    public void setRGB(int x, int y, int color) {
        ...
        this.pixels[0][x][y] = pixelColor.getRed();
        this.pixels[1][x][y] = pixelColor.getGreen();
        this.pixels[2][x][y] = pixelColor.getBlue();
    }

    public int getRGB(int x, int y) {

        int red = this.pixels[0][x][y];
        int green = this.pixels[1][x][y];
        int blue = this.pixels[2][x][y];
